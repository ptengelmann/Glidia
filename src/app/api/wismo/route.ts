...
const BodySchema = z.object({ orderName: z.string().min(1) });

export async function POST(req: NextRequest) {
  try {
    const json = await req.json();
    console.log("WISMO BODY:", json); // <— see server logs
    const { orderName } = BodySchema.parse(json);

    const data = await getOrderStatusByName(orderName);
    if (!data) {
